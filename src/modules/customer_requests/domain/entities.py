...
from src.modules.customer_requests.domain.exceptions import (
    ...
    CustomerRequestItemInvalidCurrencyError,
    CustomerRequestItemInvalidPriceError,
    CustomerRequestItemInvalidQuantityError,
    CustomerRequestItemInvalidTitleError,
...
class CustomerRequestItem:
    ...
    def __post_init__(self):
        ...
        if self.quantity <= 0:
            raise CustomerRequestItemInvalidQuantityError(
        ...
        if not title:
            raise CustomerRequestItemInvalidTitleError(
                "Product title snapshot cannot be empty"
            )
        if len(title) > 255:
            raise CustomerRequestItemInvalidTitleError(
                "Title snapshot too long (max 255)"
            )
...
class CustomerRequest:
    ...
    def create(
        ...
        if person_count is not None and not 1 <= person_count <= 500:
        ...
        if request_type != CustomerRequestType.PREORDER and items:
